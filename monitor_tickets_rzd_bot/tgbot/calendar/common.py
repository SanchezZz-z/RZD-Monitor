...
async def get_user_locale(from_user: User) -> str:
    ...
    return locale.locale_alias[loc].split(".")[0]  # + ".UTF-8"  # На сервере надо удалить .split(".")[0]
...
class GenericCalendar:
    ...
    def __init__(
        ...
        if locale:
            ...
            with calendar.different_locale(locale):
                self._labels.days_of_week = [day_abbr.capitalize() for day_abbr in calendar.day_abbr]
                self._labels.months = [month_abbr.capitalize() for month_abbr in calendar.month_abbr[1:]]
    ...
    async def process_day_select(self, data, query):
        ...
        if self.min_date and self.min_date.date() > date.date():
            await query.answer(
                "Вы не можете выбрать дату раньше сегодняшней ({})".format(self.min_date.strftime("%d/%m/%Y")),
                show_alert=self.show_alerts
            )
            return False, None
        elif self.max_date and self.max_date.date() < date.date():
            await query.answer(
                "Выберете дату не позднее {}".format(self.max_date.strftime("%d/%m/%Y")),
                show_alert=self.show_alerts
            )
            return False, None
